...
// ============ Industry Colors ============
// Industry has ~150 possible values (INDUSTRY_TAXONOMY) — too many for a hand-picked
// categorical palette. Each industry gets a shade of its parent sector's color, so the
// industry breakdown visually clusters by sector family. Labels are always shown
// alongside the swatch (DistributionList), so identity never relies on color alone.
...
/**
 * Color for an industry slice, derived as a shade of its parent sector's color.
 * `siblingIndustries` should be the full ordered industry list for that sector
 * (INDUSTRY_TAXONOMY[sector]) so shades are assigned deterministically.
 */
export function getIndustryColor(
  industry: string,
  sector: string | undefined | null,
  siblingIndustries: readonly string[] = [],
): string {
  const baseHex = (sector && SECTOR_COLORS[sector]) || SECTOR_COLORS.Other;
  const [h, s] = hexToHsl(baseHex);
  const idx = Math.max(0, siblingIndustries.indexOf(industry));
  const count = Math.max(siblingIndustries.length, 1);
  const lightness = 35 + (idx / Math.max(count - 1, 1)) * 30; // 35%–65% band
  return hslToHex(h, Math.max(s, 40), lightness);
}

...
from app.api.endpoints import market, portfolio, stocks, watchlists, options, push, insider, alerts, ai_research, ai_alerts, ai_portfolio, ai_watchlist, ai_stock_metadata, mcp, feed, journal, daily_news
...
async def unhandled_exception_handler(request: Request, exc: Exception):
    """
    Catch-all so unhandled errors still return a normal response through
    CORSMiddleware. Without this, Starlette's default 500 bypasses the
    middleware's header injection and the frontend sees a misleading CORS
...
app.include_router(ai_stock_metadata.router, prefix="/api/ai", tags=["AI Stock Metadata"])
app.include_router(mcp.router, prefix="/api/mcp", tags=["MCP"])
app.include_router(feed.router, prefix="/api/feed", tags=["Feed"])
app.include_router(journal.router, prefix="/api/journal", tags=["Journal"])
app.include_router(daily_news.router, prefix="/api/daily-briefing", tags=["Daily Briefing"])
